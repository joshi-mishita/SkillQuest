...
public:
    struct GrowthParams {
        ...
        std::vector<double> skillBonus; // bonus per skill added per year
        int years;
    };

    // Predict salary over N years using DP
    // dp[y] = max cumulative salary at end of year y
    static std::vector<double> predict(const GrowthParams& p) {
        std::vector<double> dp(p.years + 1, 0.0);
        dp[0] = p.baseSalary;
        for (int y = 1; y <= p.years; y++) {
            ...
            dp[y] = std::max(organic, dp[y-1] + bonus);
        }
        return dp;
    }

    // Display growth chart
    static void displayGrowth(const std::string& role,
                               const std::vector<double>& dp) {
        std::cout << "\n  ╔══════════════════════════════════════╗\n";
        std::cout << "  ║     SALARY GROWTH PREDICTION (DP)    ║\n";
        std::cout << "  ╚══════════════════════════════════════╝\n";
        std::cout << "  Role: " << role << "\n\n";
        double maxSal = *std::max_element(dp.begin(), dp.end());
        for (int y = 0; y < (int)dp.size(); y++) {
            int bar = (int)(dp[y] / maxSal * 35);
            std::cout << "  Yr" << std::setw(2) << y << " | "
                      << std::string(bar, '#')
                      << " $" << std::fixed << std::setprecision(0) << dp[y] << "\n";
        }
    }
    ...
    suggest(const std::string& query,
